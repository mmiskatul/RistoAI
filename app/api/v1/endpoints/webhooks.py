...
def _billing_cycle(product_id: str) -> SubscriptionPlan:
    ...
    annual_markers = ('annual', 'annually', 'year', 'yearly', '12_month', '12-month')
    return SubscriptionPlan.ONE_YEAR if any(marker in product for marker in annual_markers) else SubscriptionPlan.ONE_MONTH
...
async def handle_revenuecat_webhook(
    ...
    inactive_events = {'CANCELLATION', 'EXPIRATION', 'REFUND'}
    ...
    if event_type in active_events:
        ...
        update_data = {
            ...
            'subscription_plan_name': settings.subscription_plan_name,
        ...
        update_data = {
            'is_pro': False,
            'subscription_status': SubscriptionStatus.EXPIRED if event_type in {'EXPIRATION', 'REFUND'} else SubscriptionStatus.CANCELED,
            'subscription_expires_at': _expiration(event.get('expiration_at_ms')),
            'updated_at': now,
        }
    else:
